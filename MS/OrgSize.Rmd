...
and large organisms differ in population size, growth rates, morphological
...
smallest disperse the furthest [@DeBie2012a]. This is the reasoning behind the classic "everything is everywhere and the
...
long distances by wind, water, or phoresy. In contrast, larger
...
(full shade). We also performed a second experiment using only _N. cruenta_. This is possible because this bromeliad (unlike the other two species) is found in many habitats. While the first experiment used only _N. cruenta_ growing in full shade, in the second experiment we used _N. cruenta_ from full sun, partial shade and full shade. Thus we are able to separate the effects of bromeliad species from the effects of the habitats where they are found.

```{r start_end_dates}
start_end <- function(exp){
  blocks %>% 
...
str_end <- start_end("threespp")

str_end_win <- start_end("withinsp")
```

In both the three-bromeliad and _N. cruenta_ experiments, our goal was
to create identical starting community composition for all bromeliads within a
block. 
To set up one experimental block we used six bromeliads. 
In the three-bromeliad experiment we collected and sampled the macroinvertebrates, zooplankton and bacteria of two
bromeliads of each of the three species; in the _N. cruenta_ experiment we sampled three individuals from each of open and closed habitats. We then homogenized the communities
of all six bromeliads as described shortly (Figure 2).  
Blocks varied in the timing of their setup, their spatial location, and slightly in starting community composition; this variation is
included in the random effect of blocks. We created five blocks in the three-bromeliad experiment
experiment between `r format(str_end[[1]], format = "%d %B %Y")` and `r format(str_end[[2]], format = "%d %B %Y")`; and 
another five in the _N. cruenta_ experiment between `r format(str_end_win[[1]], format = "%d %B %Y")` and `r format(str_end_win[[2]], format = "%d %B %Y")`
...
confirmed that this was sufficient time for communities to change, but not so long that bromeliads became empty of macroscopic organisms.
...
<!-- Code to reproduce the analysis published here is available
at ***link to data*** -->


## Results   
```{r gettime, echo = FALSE}
...
Both before and at the end of our experiment, bromeliad species identity explains more variation in community composition of macroinvertebrates than any other organism type, less for zooplankton and less still for bacteria (Figure 3, Table 1). This pattern was repeated for the _N. cruenta_ experiment. In both experiments and for all organism types, communities were more different before the experiment (i.e., in nature) than an the end. Note that, although the sampling design (and therefore degrees of freedom) are identical for all groups, the critical (alpha = 0.05) F-value for each organism group differs because PERMANOVA p-values are calculated on a null distribution generated by permuting samples among groups (species in our case). Bacterial communities have many
...
**Figure 3**: The amount of variation (R^2^ from PERMANOVA) in community composition explained by bromeliad species (i.e. the strength of the environmental signal) increases with the approximate body size of organisms.  The environmental signal in initial, undisturbed communities was removed by homogenization, but after 12 days of recovery, was again of similar strength in final macroinvertebrate and bacterial communities. Inset shows the results of a null simulation to test the significance of this increase in R^2^ value: histogram is the distribution of slopes (i.e. change in R^2^ as a function of relative organism size) and the dark line indicates the observed value of the slope for observations at the end of the experiment (i.e. black dots).
...
```{r figure_1, fig.cap=""}
knitr::include_graphics("../photos/hypotheses_illustration.pdf")
```
\newpage
...
```{r figure_2, fig.cap=""}
knitr::include_graphics("../photos/design_illustration.pdf")
```
\newpage
...
```{r figure_3, fig.cap=""}
knitr::include_graphics("../figures/r2_plot.pdf")
```
