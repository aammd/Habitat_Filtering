...
*Question 1: less beta diversity among smaller taxa*
The duration of our experiment is probably not enough time for our invertebrate communities to recover the 

If smaller taxa are more plastic, they are unlikely to develop strong inter-
patch differences during our experiment, or will do so more slowly than larger
organisms. We tested this by comparing homogeniety of multivariate
dispersions [@Anderson2006] between all six bromeliads (i.e. within one block) before and after the experiment. We repeated this comparison for all five blocks, and for all thee organism types. We used the `betadisper` function in the R package
vegan [@vegan], which includes a permutation test to test for differences before and
after the experiment within each block (999 null simulations block^-1^). All
statistical analyses were conducted in R [@rcore]. We tested question 2 with a
permutational ANOVA (PERMANOVA) to detect differences in the degree of
environmental signal among groups. PERMANOVA measures the amount of difference
in community composition between treatment groups and compares this to the
expected distribution under a null hypothesis of no treatment effects. If
larger-bodied groups of organisms are more filtered, then we expect to find a
greater difference in community composition emerge among the
macroinvertebrates, less among the zooplankton, and little to none among
bacteria. We complemented this approach in question 3 with a multivariate
regression of abundance [@mvabund] to identify those taxa which contributed
most to the differences.

We used a technique called multivariate generalized linear models
[@Warton2012] from the R package mvabund [@Wang2012;@mvabund]. This approach
has three advantages that make it suitable for our application. First, because
it is based on GLMs, we are able to model the mean-variance relationship
common in count data. Distance-based metrics, on the other hand, can conflate
differences in dispersion with differences in location [@Warton2012]. We are
already testing for dispersion and location effects in the separate analysis
described above with distance based methods. Therefore this multivariate GLM
approach helps confirm our findings by accounting for both within a different
framework. Second, it allows us to test for an interaction between our
treatments (different bromeliad species) and blocks. Third, mvabund also
allows for estimating individual species-level responses. These are tested
using bootrap tests that correct for multiple comparisons. This technique lets
us identify which species responded most to the environmental differences, and
to relate these to traits of these species.
