...
(i.e. the slopes of lines in `r fig("disp")`) declines with decreasing organism
...
`r tab_nums("perm_3sp", "Bromeliad species effects on the composition of three groups of organisms, as determined by PERMANOVAs both before and 12 days after homogenization. Both F-ratios and R^2^ values are higher for macroinvertebrates than for zooplankton than for bacteria (before and after). Following homogenization, macroinvertebrate and bacterial communities both significantly diverged between bromeliad species. ")`
...
`r fig_nums("disp", "Mean distance to centroid, before and after the experiment, for each of the three organism types. Each dot represents the mean distance to centroid for all six bromeliads in a block, and lines connect initial and final measurements of the same block.")`
...
interaction (`r tab("disp_tab")`). However, this method assumes that
