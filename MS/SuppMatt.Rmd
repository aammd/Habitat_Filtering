...
```{r}
make_disp_anova <- function(make_betadisp_output){
  make_betadisp_output %>% anova %>% broom::tidy(.)  %>% knitr::kable()
}
```
...
```{r}
### macroinvertebrates
make_disp_anova(inverts_disper_species)
### zooplankton
make_disp_anova(zoops_disper_species)
### bacteria
make_disp_anova(bact_disper_species )
```
