...
distinct processes: dispersal, selection, speciation and drift. In this
framework, dispersal refers to the arrival of species into a local community,
“selection” to differences in colonization success or growth rate among
species, and “drift” to changes in species abundances that may be the result
of stochastic processes. These processes together can describe all of the
contemporary models of community ecology. For example, neutral theory assumes
all individuals of all species are equivalent, and so is based on dispersal,
drift and speciation along. In a niche-based framework, sSpecies are able to
coexist if they possess traits which permit them to colonize, and if they are
sufficiently different from other species to allow coexistence.


### Macroscopic organisms and microscopic organisms show different patterns of distribution -- plasticity

Macroscopic organisms and microscopic organisms show different patterns of
distribution. Microscopic organisms are often globally distributed, while
macroscopic organisms have more geographically restricted distributions. For
example, plankton community similarity is correlated with differences in
environmental conditions [@Huszar2015]. Observations on each group independently
have shown patterns of different strengths, (cite). Our own field observations
have shown that environmental variation has the strongest effects on
macroinvertebrates, weaker on zooplankton, and weaker still on bacteria (cite).
Why should the strength of environmental filtering decrease with organism size?
There are two possible mechanisms that may make communities of smaller organisms
more strongly filtered. First, smaller organisms have larger environmental
tolerances, and therefore can live in a broader fundamental niche. Second,
smaller organisms have larger dispersal ability, allowing them to be present in
more communities.
...
#Explanation 2:
Microscopic organisms may be able to disperse further than
macroscopic organisms. The classic "everything is everywhere and the
environment selects" hypothesis of [cite] suggests that smaller organisms are not
limited by biogeographic barriers but instead are found everywhere, emerging from resistant stages in favorable environments. Many bacteria and zooplankton have passive dispersal,
traveling between suitable habitat by wind or water currents, or by phoresy.
In contrast, macroscopic organisms usually have active dispersal, for example
oviposition by flying adults. At smaller scales, this could result in a close
association between distribution and environmental variables, assuming that
active dispersal is adapted to maximize fitness. However, at larger scales
barriers to active dispersal might prevent macroscopic animals from reaching
suitable places. This would weaken the association between environment and
distribution. Dispersal is higher for smaller animals both in terms
of the number of dispersing propagules (because population sizes are so much
larger) and the distance dispersed (since passive dispersal requires no
energy, organisms can go very far indeed.) Environmental filtering or
...
presence of a predator. Previous researchers have used RDA () to separate the
effects of space, but this approach still has limitations. Patterns of
dispersal may be irregular and nonlinear, caused by individual behaviour
(active dispersers) or by nonlinear, unpredictable vectors (passive
dispersers). An experiment that removes dispersal limitation for all groups is
a stronger test of the relative effects of environment on species composition.
We are aware of no study that experimentally removes dispersal limitation for
both micro- and macro-organisms in the same system, simultaneously.

#Bromeliads 
We conduct an experiment that removes dispersal limitation for
...
show that this variation is more closely associated with variation in
macroinvertebrate composition, whereas bacteria communities show no signal of
the environment (Farjalla et al.).

#Hypotheses
Here we provide a much stronger test of the strength of environmental filtering for these three organism groups by experimentally dispersing all species to all habitats.  We had three hypotheses about the outcome: 
1). If environmental filtering increases with organism size, we would predict that larger organisms would show stronger habitat effects on community composition than smaller organisms, regardless of our experimental dispersal treatment (Fig. 1a). 
...
along the same gradient of environmental variation. The location we used is
the Parque Nacional de Jurubatiba, Northeast Rio de Janeiro state, Brazil
($22^{\circ}$ S $41^{\circ}$); the environmental gradient is twofold -- three
different species of bromeliad, which grow in three levels of exposure to
sunlight. Each species of bromeliad is found in a different habitat of the
...
full sun; only shade plants were used in this study. 
...
species -- sampled them, and mixed their communities together (see Figure  1, description of sampling below.).  Our goal was to create identical starting community composition for all bromeliads within a block. Variation between blocks in starting community composition is thus included in the random effect of blocks. We created five blocks in this experiment between 2013/03/27 and 2013/04/03.

![img](../photos/text_design.png) Figure  1: A schematic of our
...
Our experimental setup consisted of three steps (Figure  1): (1) initial
...
(i.e. the slopes of lines in Figure  2) decline with decreasing organism
